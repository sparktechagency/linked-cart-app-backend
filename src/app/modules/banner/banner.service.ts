...
const updateBannerToDB = async (id: string, payload: IBanner): Promise<IBanner | {}> => {
  ...
  checkMongooseIDValidation(id, "Banner");
...
const deleteBannerToDB = async (id: string): Promise<IBanner | undefined> => {
  ...
  const deleteBanner = await Banner.findByIdAndUpdate(
    id,
    { status: "Delete" },
    { new: true }
  );
  ...
  if(!deleteBanner){
    throw new ApiError(StatusCodes.BAD_REQUEST, "Banner Deleted Successfully")
  }
