...
const loginUserFromDB = async (payload: ILoginData) => {
    ...
    const rolesRequiringDeviceToken = [USER_ROLES.SHOP, USER_ROLES.CUSTOMER, USER_ROLES.SHOPPER];
    if (rolesRequiringDeviceToken.includes(isExistUser?.role) && !deviceToken) {
        throw new ApiError(StatusCodes.UNAUTHORIZED, "Invalid Device token");
    }
    ...
    if (
        isExistUser?.role === USER_ROLES.SHOP ||
        isExistUser?.role === USER_ROLES.CUSTOMER ||
        isExistUser?.role === USER_ROLES.SHOPPER
    ) {
